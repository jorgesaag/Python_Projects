...
#Write a function called check_nums that takes a list as its parameter, and contains a while loop that only
#stops once the element of the list is the number 7.
...
print(check_nums([2, 3, 4, 5, 6, 7, 8, 9]))
...
def sublist(lststr):
    sblst = []
    index = 0
    while str(lststr[index]) != "STOP":
        sblst.append(str(lststr[index]))
        index += 1
    return sblst
    print(sblst)

print(sublist(["if", 4, "do", 5, "STOP"]))
...
def stop_at_z(lststr):
    newlst = []
    index = 0
    while str(lststr[index]) != "z":
        newlst.append(str(lststr[index]))
        index += 1
    return newlst
    print(newlst)

print(stop_at_z([1, "e", "r", 0, 1, "z", 6, "q", 8]))
...
print(beginning([1, "e", "r", 0, 1, "if", 4, "bye", 5, "STOP", "z", 6, "q", 8]))