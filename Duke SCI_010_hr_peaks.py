...
def tests_for_hr_data(data_and_answers, w):
    ...
    for (data, answers) in data_and_answers:
        ...
        if (count == len(answers)):
            ...
            print("**INCORRECT** lap count, got " + str(count) + " but expected " + str(len(answers)))
            all_right = False
            pass
        pass
    return all_right
...
if __name__ == "__main__":
    ...
    data3 = []
    peaksat3 = []
    # A one element list which has no peaks (we require at least w=2 points on each side)
    data4 = [190]
    peaksat4 = []
    # Still to few points for a peak with w = 2
    data5 = [188, 190, 189]
    peaksat5 = []
    # The smallest list we can make with a peak for w = 2
    data6 = [187, 188, 189, 186, 180]
    peaksat6 = [2]
    # We can make a list of tuples for the data we want to test with for w = 2
    w2_data = [(data0, peaksat0), (data1, peaksat1), (data2, peaksat2), (data3, peaksat3), (data4, peaksat4),
               (data5, peaksat5), (data6, peaksat6)]
    all_right = tests_for_hr_data(w2_data, 2)
    w1_data = w2_data[:]
    w1_data[1] = (data1, [2, 6, 8])
    w1_data[5] = (data5, [1])
    all_right = tests_for_hr_data(w1_data, 1) and all_right
    w4_data = [(data0, []), (data1, []), (data2, [4]), (data3, []), (data4, []), (data5, []), (data6, [])]
