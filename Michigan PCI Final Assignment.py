...
#Players see the category and an obscured version of the phrase where every alphanumeric character in the phrase
#starts out as hidden (using underscores: _):
...
#During their turn, every player spins the wheel to determine a prize amount and:
    ...
    #If the wheel lands on a cash square, players may do one of three actions:
        ...
        #Guess any letter that hasn’t been guessed by typing a letter (a-z)
             ...
             #The player can guess any letter that hasn’t been guessed and gets that cash amount for every time that
    ...
    #f the wheel lands on “bankrupt”, the player loses their turn and loses their money but they keep all of the
...
#functions and methods that you may not be aware of. The active code window that starts with “Part A” is where you
#are first asked to complete code.
...
#Here is the final code for the Wheel of Fortune in Python
        ...
        for letter in LETTERS:
            if self.prizeMoney >= VOWEL_COST:
                ...
                if letter not in guessed and letter not in VOWELS:
                    possible_letters.append(letter)
        possible_letters = "".join(map(str, possible_letters))
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        if self.prizeMoney >= VOWEL_COST:
            if self.smartCoinFlip() == False:
                ...
                return random.choice([letter for letter in self.SORTED_FREQUENCIES if letter not in guessed])
        else:
            if all(letter in VOWELS for letter in self.getPossibleLetters(guessed)) == True:
                return "pass"
            else:
                if self.smartCoinFlip() == False:
                    for letter in self.SORTED_FREQUENCIES[::-1]:
                    ...
                    return random.choice([letter for letter in self.SORTED_FREQUENCIES if letter not in guessed and letter not in VOWELS])
