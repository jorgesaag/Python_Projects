#A common problem, especially when dealing with data returned from a web site, is to extract certain elements from
#deep inside a nested data structure. In principle, there’s nothing more difficult about pulling something out
#from deep inside a nested data structure: with lists, you use [] to index or a for loop to get them them all;
#with dictionaries, you get the value associated with a particular key using [] or iterate through all the keys,
#accessing the value for each. But it’s easy to get lost in the process and think you’ve extracted something different
#than you really have. Because of this, we have created a usable technique to help you during the debugging process.
...
#To illustrate this, we will walk through extracting information from data formatted in a way that it’s return by
#the Twitter API. This nested dictionary results from querying Twitter, asking for three tweets matching
#“University of Michigan”. As you’ll see, it’s quite a daunting data structure, even when printed with nice
...
#At any level of the extraction process, the first task is to make sure you understand the current object you have extracted.
#There are few options here.

#1) Print the entire object. If it’s small enough, you may be able to make sense of the printout directly.
#If it’s a little bit larger, you may find it helpful to “pretty-print” it, with indentation showing the level of
#nesting of the data. We don’t have a way to pretty-print in our online browser-based environment,
#but if you’re running code with a full Python interpreter, you can use the dumps function in the json module.
#For example:
...
#If it’s a dictionary, figure out which key has the value you’re looking for, and get its value.
#For example: res2 = res['statuses']
#If it’s a list, you will typically be wanting to do something with each of the items
#(e.g., extracting something from each, and accumulating them in a list).
#For that you’ll want a for loop, such as for res2 in res.
#During your exploration phase, however, it will be easier to debug things if you work with just one item.
#One trick for doing that is to iterate over a slice of the list containing just one item.
#For example, for res2 in res[:1].
...
#Now extract. Since it’s a list, we’ll want to work with each item, but to keep things manageable for now,
#let’s use the trick for just looking at the first item. Later we’ll switch to processing all the items.
