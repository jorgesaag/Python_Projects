#The code below takes the list of country, country, and searches to see if it is in the dictionary gold which
...
#each number in numb. For example, the first element should be 0, because 36/6 has no remainder.
...
#Use try/except so that the code will run properly. If the sport is not in the dictionary, ppl_play, add it in
#with the value of 1.
...
#Provided is a buggy for loop that tries to accumulate some values out of some dictionaries.
#Insert a try/except so that the code passes. If the key is not there, initialize it in the dictionary and set
#the value to zero.
