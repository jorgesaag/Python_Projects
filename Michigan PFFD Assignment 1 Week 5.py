...
keys = medals.keys()
top_three = sorted(keys, key=lambda x: medalcount(x, medals), reverse=True)[:3]
print(top_three)
...
keys = groceries.keys()
most_needed = list(sorted(keys, key=lambda x: itemcount(x, groceries), reverse=True))
print(most_needed)
...
sorted_ids = sorted(ids, key=lambda x: last_four(ids))
print(sorted_ids)
